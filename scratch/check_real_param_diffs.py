        ...
        def compare_dicts(d1, d2, prefix=""):
            for k in d1:
                ...
                if isinstance(v1, torch.Tensor):
                    diff = torch.max(torch.abs(v1 - v2)).item()
                    print(f"Tensor {p} max diff: {diff}")
                elif isinstance(v1, dict):
                    compare_dicts(v1, v2, prefix=p)

        print("\n=== RECURSIVE PARAMETER DIFFERENCES AT STEP 100 ===")
        compare_dicts(ckpt_un, ckpt_in)
