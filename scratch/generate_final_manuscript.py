...
We situate our work within continual learning, plasticity preservation, and offline-to-online RL (detailed conceptual matrix provided in Appendix Table~\ref{tab:lit_comparison}).

\paragraph{Loss of Plasticity in Deep Continual Learning}
The degradation of gradient-based learning capacity in deep neural networks has emerged as a fundamental challenge \citep{lyle2023understanding, dohare2024loss, lyle2024plasticity}. In continual supervised learning, networks trained over extended horizons suffer from weight norm explosion, dead activations, and rank collapse. \citet{dohare2024loss} demonstrated that standard backpropagation cannot sustain lifelong adaptation and proposed Continual Backpropagation (CBP) to continually reinitialize low-utility units. In contrast to millions of gradient steps in lifelong learning, our work examines offline-to-online RL over tens of thousands of fine-tuning steps, finding that representations remain remarkably resilient in stable transfer and that unneeded intervention poses an immediate hazard.

\paragraph{Implicit Under-Parameterization and Rank Collapse}
\citet{kumar2020implicit} demonstrated that temporal-difference (TD) learning in deep Q-networks implicitly under-parameterizes feature representations, as bootstrapping collapses feature covariance matrices onto a low-dimensional subspace (low effective rank, or sRank). In response, \citet{kumar2022dr3} proposed DR3, an explicit feature dot-product regularization penalty during offline pretraining. While DR3 acts as a continuous loss regularizer, our work explores whether relative effective rank retention can function as an active closed-loop gating signal to govern discrete interventions during online adaptation.

\paragraph{Resetting and Perturbation Operators}
To alleviate primacy bias---where agents overfit to early replay experiences---\citet{nikishin2022primacy} proposed periodic network resets, achieving substantial sample-efficiency gains in Atari 100k and continuous control from scratch \citep{doro2023sample}. In continual supervised learning, \citet{ash2020warmstarting} proposed Shrink-and-Perturb (SP): shrinking weights toward the origin and injecting Gaussian noise to escape local minima. While these methods succeed when early representations are low-quality, we show that in O2O continuous control, perturbing converged representations destroys delicate dynamical equilibria.

\paragraph{Dormant Neuron Recycling}
\citet{sokar2023dormant} characterized the dormant neuron phenomenon in deep RL, showing that many ReLU units become permanently inactive during training. They proposed ReDo (Recycling Dormant Neurons), which reinitializes dormant units while explicitly setting outgoing weights to zero ($W_{\mathrm{out}} = 0$). \citet{schwarzer2024bigger} scaled this principle to Atari 200M. Crucially, ReDo possesses the mathematical property of \emph{zero functional shift} at the instant of intervention ($f_{\theta'}(x) \equiv f_\theta(x)$). In Section~\ref{sec:results}, we demonstrate that this zero-functional-shift property is decisive for maintaining stability in continuous control under non-stationary physical stress.

\paragraph{Offline-to-Online Reinforcement Learning}
Offline-to-online RL bridges static pretraining and interactive online adaptation \citep{nair2020awac, lee2022offline2online, kostrikov2021iql, ball2023efficient}. Conservative algorithms like Implicit Q-Learning (IQL) \citep{kostrikov2021iql} avoid out-of-distribution action queries during offline pretraining. When fine-tuning online, methods balance replay mixing ratios or decay policy constraints. Our study builds on canonical IQL to conduct a systematic investigation into how parameter interventions interact with converged representations in continuous control.

\paragraph{Conceptual Positioning}
Existing techniques fall into open-loop interventions (SP, ReDo, Resets scheduled every $K$ steps without checking representation health) or continuous observational metrics without intervention triggers (sRank). \textsc{CapacityGate} introduces closed-loop diagnostic gating: it evaluates both dimensional compression ($\rho$) and unresponsiveness ($d$), abstaining when representations remain healthy and intervening only upon diagnostic confirmation of capacity loss.
...
We consider a Markov Decision Process (MDP) defined by the tuple $\mathcal{M} = (\mathcal{S}, \mathcal{A}, \mathcal{P}, \mathcal{R}, \gamma)$. During offline pretraining, the agent accesses a static dataset $\mathcal{D}_{\mathrm{off}} = \{(s_i, a_i, r_i, s'_i)\}_{i=1}^N$ collected by unknown behavioral policies. The agent optimizes parameters $\theta = (\phi, \psi, \omega)$ for policy $\pi_\phi(a|s)$, state-value function $V_\psi(s)$, and twin action-value functions $Q_\omega(s, a)$ via Implicit Q-Learning (IQL) \citep{kostrikov2021iql}. 

Upon completing offline pretraining, the agent transitions to interactive online adaptation. At each online environment step $t$, the agent acts in the environment, observes transition $(s_t, a_t, r_t, s_{t+1})$, and appends it to an online replay buffer $\mathcal{D}_{\mathrm{on}}$. Model updates sample mini-batches with a 50\% offline and 50\% online mixing ratio to prevent distributional collapse \citep{lee2022offline2online}.

\subsection{Representation Capacity Diagnostics}
\label{sec:diagnostics}

To measure representation health without perturbing ongoing gradient updates, \textsc{CapacityGate} computes two complementary non-invasive metrics:

\paragraph{Relative Effective Rank Ratio ($\rho$)}
Let $H \in \mathbb{R}^{B \times D}$ denote the matrix of activations from the penultimate feature layer across a probe batch of size $B$. Following \citet{roy2007effective} and \citet{kumar2020implicit}, the effective rank $s_{\mathrm{eff}}(H)$ is defined via the singular values $\sigma_1 \ge \sigma_2 \ge \dots \ge \sigma_K > 0$ of $H$ (where $K = \min(B, D)$). We compute normalized singular values $p_k = \sigma_k / \sum_{i=1}^K \sigma_i$ and evaluate the exponential of the Shannon entropy:
\begin{equation}
    s_{\mathrm{eff}}(H) = \exp\left( -\sum_{k=1}^K p_k \ln p_k \right).
\end{equation}
Because absolute effective rank varies across network architectures and environments, \textsc{CapacityGate} evaluates the \emph{relative effective rank ratio} $\rho(t)$:
\begin{equation}
    \rho(t) = \frac{s_{\mathrm{eff}}(H_t)}{s_{\mathrm{anchor}}},
\end{equation}
where $s_{\mathrm{anchor}} = s_{\mathrm{eff}}(H_0)$ is computed on the initial online probe batch using the converged offline weights. A value of $\rho(t) \approx 1.0$ indicates preserved dimensional span, whereas $\rho(t) \ll 1.0$ indicates dimensional collapse.

\paragraph{Activation Dormancy Fraction ($d$)}
Complementing global rank, activation dormancy measures the proportion of inactive units. Following \citet{sokar2023dormant}, a neuron $j$ in layer $l$ is defined as dormant with respect to probe batch $\mathcal{B}_{\mathrm{probe}}$ if its average post-activation score is non-positive:
\begin{equation}
    a_j^l = \frac{1}{B} \sum_{i=1}^B \max\left(0, z_{i,j}^l\right) \le 0,
\end{equation}
where $z_{i,j}^l$ is the pre-activation of neuron $j$ for input $x_i$. The layer dormancy $d_l$ is the fraction of dormant units: $d_l = \frac{1}{M_l} \sum_{j=1}^{M_l} \mathbb{I}(a_j^l \le 0)$, and overall network dormancy $d(t)$ is the mean across all hidden layers.

\subsection{Probe Sampling: Uniform Replay Reservoir vs. Temporal Slices}
\label{sec:probe_sampling}

A critical methodological finding of this work is that \textbf{diagnostic telemetry is sensitive to probe batch construction}. If probe batches are constructed from recent transitions (e.g., the last $B$ steps in $\mathcal{D}_{\mathrm{on}}$), temporal autocorrelation introduces severe state-space localization. In locomotion, consecutive states traverse narrow kinematic sub-phases (e.g., left-foot support), temporarily inactivating units responsible for alternate phases. This produces artificial dormancy spikes (spurious readings up to $d \approx 0.35$).

To eliminate this autocorrelation artifact, \textsc{CapacityGate} enforces \emph{uniform reservoir sampling} across the full online buffer:
\begin{equation}
    \mathcal{B}_{\mathrm{probe}} \sim \mathrm{Uniform}\left(\mathcal{D}_{\mathrm{on}}[0:t]\right).
\end{equation}
Uniform reservoir sampling reflects the true stationary state distribution, maintaining dormancy below $0.085$ under healthy adaptation and preventing spurious controller triggers.

\subsection{Intervention Operators: Perturbation vs. Zero-Functional Shift}
\label{sec:operators}

When an intervention is triggered, \textsc{CapacityGate} can deploy two distinct parameter-level operators:
\begin{itemize}
    \item \textbf{Shrink-and-Perturb (SP)} \citep{ash2020warmstarting}: Parameters $\theta$ undergo shrinkage toward the origin followed by Gaussian noise injection:
    \begin{equation}
        \theta' = (1 - \lambda)\theta + \sigma \cdot \epsilon, \quad \epsilon \sim \mathcal{N}(0, I),
    \end{equation}
    with $\lambda = 0.10$ and $\sigma = 0.10$. Crucially, SP induces an immediate non-zero functional shift: $\Delta f_\theta(x) = f_{\theta'}(x) - f_\theta(x) \neq 0$.
    \item \textbf{Neuron Recycling (ReDo)} \citep{sokar2023dormant}: Units identified as dormant ($a_j^l \le 0$) have incoming weights reinitialized via Kaiming normal, while outgoing weights are explicitly set to zero ($W_{\mathrm{out}} = 0$). Adam moments for recycled units are cleared. ReDo preserves function identically at the moment of intervention: $f_{\theta'}(x) \equiv f_\theta(x)$.
\end{itemize}

\subsection{Closed-Loop Control: Hysteresis and Refractory Cooldown}
\label{sec:controller}

Unconstrained intervention induces destructive thrashing. \textsc{CapacityGate} governs actuation through dual-threshold hysteresis and refractory cooldown:

\paragraph{Hysteresis Gating} Let $g_t \in \{0, 1\}$ denote the binary gate state at check step $t$ (evaluated every $\Delta t = 1,000$ steps). The gate activates when either rank collapses or dormancy surges, and deactivates only when both metrics recover:
\begin{equation}
    g_t = \begin{cases}
    ...
    \end{cases}
\end{equation}
with $\rho_{\mathrm{on}} = 0.70$, $\rho_{\mathrm{off}} = 0.85$, $d_{\mathrm{on}} = 0.15$, and $d_{\mathrm{off}} = 0.08$.

\paragraph{Refractory Cooldown Control} When $g_t = 1$, intervention actuates only when at least $\Delta_{\mathrm{cool}} = 3,000$ online steps have elapsed since the prior intervention:
\begin{equation}
    \mathrm{Intervene}_t = \mathbb{I}\left( g_t = 1 \land (t - t_{\mathrm{last\_int}} \ge \Delta_{\mathrm{cool}}) \right).
\end{equation}
This refractory constraint prevents parameter thrashing at consecutive steps, allowing the gradient optimizer to adapt with the restored capacity. The complete protocol is formalized in Algorithm~\ref{alg:capacity_gate}.
...
We design our evaluation protocol to rigorously benchmark intervention methods in O2O continuous control.

\paragraph{Benchmark Tasks and Shift Regimes}
We evaluate on standard D4RL continuous-control locomotion tasks: \texttt{halfcheetah-medium-v2}, \texttt{hopper-medium-v2}, and \texttt{walker2d-medium-v2} \citep{fu2020d4rl}. To benchmark resilience across realistic deployment challenges, we examine four distinct distribution-shift regimes:
\begin{enumerate}
    \item \textbf{Clean Transfer (\texttt{none})}: The online environment dynamics and reward formulation match offline pretraining identically ($\sigma = 0, \alpha = 1.0$).
    \item \textbf{Sensor Observation Noise (\texttt{obs\_noise})}: Online observations are corrupted by zero-mean additive Gaussian noise $s_t \leftarrow s_t + \mathcal{N}(0, \sigma^2 I)$ with standard deviation $\sigma = 0.10$.
    \item \textbf{Reward Rescaling (\texttt{reward\_scale})}: Environment rewards are rescaled by factor $\alpha = 0.50$ ($r_t \leftarrow \alpha \cdot r_t$), altering the scale of value targets.
    \item \textbf{Non-Stationary Actuator Crippling (\texttt{actuator\_cripple})}: At online step $t = 5,000$, a critical joint actuator is abruptly disabled ($a_{\mathrm{crippled}} = 0$), inducing a severe physical non-stationarity.
\end{enumerate}

\paragraph{Paired Evaluation Protocol and Statistical Rigor}
To ensure statistical reliability, all experimental comparisons are executed under an identical, seed-matched paired design across 5 random seeds (Seeds 0--4). The confirmatory benchmark comprises 135 factorial runs ($3 \text{ environments} \times 3 \text{ shift regimes} \times 3 \text{ methods} \times 5 \text{ seeds}$), yielding $N = 45$ matched evaluations. The non-stationary stress study evaluates 24 targeted runs across 3 seeds. Every run logs 10 deterministic evaluation rollouts every 2,500 steps. Following \citet{agarwal2021deep}, aggregate scores are reported via Interquartile Mean (IQM) normalized return with 95\% stratified bootstrap confidence intervals. Statistical significance between matched pairs is assessed using the two-tailed non-parametric paired Wilcoxon signed-rank test.

\section{Empirical Results \& Analysis}
\label{sec:results}

We present empirical evidence addressing the three core research questions.
...
\paragraph{Aggregate Statistical Distribution} Across all evaluated runs, the aggregate Interquartile Mean (IQM) normalized return highlights a severe performance collapse under unconditional periodic Shrink-and-Perturb:
\begin{itemize}
...
Examining individual environments reveals that intervention vulnerability is strongly tied to task physics (Figure~\ref{fig:learning_curves}):
\begin{itemize}
    \item \textbf{Walker2d-v2 (Dynamic Bipedal Balance)}: Walker2d exhibits extreme sensitivity. Under clean transfer, unconditional periodic intervention drops normalized return from $31.45 \pm 12.65$ to $2.14 \pm 5.79$. Under reward rescaling, Baseline policies adapt to $64.33 \pm 11.22$, whereas Fixed collapses to $1.94 \pm 5.64$. In bipedal locomotion, periodic weight perturbation knocks the policy off its stable limit cycle into irreversible falling.
    \item \textbf{Hopper-v2 (Monopod Hop Stabilization)}: Under clean transfer, Fixed degrades Hopper from $40.98 \pm 2.46$ to $14.87 \pm 8.45$. Periodic perturbation repeatedly destabilizes the hopping rebound cycle.
    \item \textbf{HalfCheetah-v2 (Forward Propulsion)}: HalfCheetah exhibits no balance failure mode (it cannot fall over). Nevertheless, periodic Shrink-and-Perturb degrades return from $35.32 \pm 2.29$ to $10.71 \pm 5.02$. Perturbing parameters degrades policy velocity without providing any exploratory benefit.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Rank Retention}: Relative effective rank ratio remains exceptionally stable: $\rho(t) \in [0.99, 1.03]$. The representation experiences no rank collapse, remaining far above the trigger threshold $\rho_{\mathrm{on}} = 0.70$.
...
\begin{subfigure}[b]{0.48\textwidth}
    ...
    \label{fig:telemetry_trajectories}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.48\textwidth}
    \centering
...
\begin{itemize}
    ...
    \item \textbf{HalfCheetah Gait Adaptation (Fixed $15.26$ vs. ReDo $13.64$ vs. Baseline $6.23$)}: Unlike Walker2d, HalfCheetah cannot fall over. Disabling the rear actuator severely impedes forward velocity under the offline prior ($6.23 \pm 1.53$ baseline). In this unconstrained forward propulsion task, empirical observations indicate that disrupting the offline prior via weight perturbation or neuron recycling is consistent with discovering alternate front-joint locomotion modes, enabling Fixed ($15.26 \pm 2.93$) and ReDo ($13.64 \pm 3.18$) to substantially outperform the passive baseline.
\end{itemize}
...
To facilitate reproducibility, all algorithmic implementations, evaluation protocols, environment configurations, and random seeds are fully documented in Section~\ref{sec:method}, Section~\ref{sec:setup}, and Appendix Table~\ref{tab:hyperparams}. The complete codebase, configuration files, trained model checkpoints, and raw evaluation JSON logs are made available in the supplementary repository. All reported statistical metrics (IQM, Wilcoxon signed-rank tests, stratified bootstrap confidence intervals) adhere strictly to the open-source \texttt{rliable} protocol \citep{agarwal2021deep}.
